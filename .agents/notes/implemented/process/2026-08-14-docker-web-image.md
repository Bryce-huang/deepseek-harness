...
`docker-build.yml` publishes to GHCR with the built-in `GITHUB_TOKEN` (no registry secrets). Master pushes publish `latest`, the UTC build date (`YYYY.MM.DD`), and `sha-<short>`; `v*` and `dsh-v*` tags publish the tag name. The repository name is lowercased because GHCR requires lowercase image paths. Builds are amd64 with the GitHub Actions cache, matching the wehermes workflow structure this workflow was modeled on.
